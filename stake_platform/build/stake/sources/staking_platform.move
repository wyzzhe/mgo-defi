...
module stake::staking_platform {
    ...
    public struct StakingPlatform<phantom COIN> has store, key {
        ...
        update_stakingplatform_event_counter: u64,
    ...
    public entry fun create_staking_platform<COIN>(
        ...
        let staking_platform = StakingPlatform {
            ...
            update_stakingplatform_event_counter: 0,
            stake_address_record_table: table::new<address, Table<u64, StakeInfo>>(ctx),
        };

        let currency_type = type_name::get<COIN>();

        event::emit(CreateStakePlatformEvent {
            event_counter: staking_platform.create_stakingplatform_event_counter,
    ...
    public entry fun update_staking_platform<COIN>(
        ...
        event::emit(UpdateStakePlatformEvent {
            event_counter: staking_platform.update_stakingplatform_event_counter,
            admin: ctx.sender(),
            annual_rate,
        });

        staking_platform.update_stakingplatform_event_counter = staking_platform.update_stakingplatform_event_counter + 1;
    }
    ...
    public entry fun withdraw_reward<COIN>(
        ...
        // 借用并修改提取时间
        let mut stake_address_record_table = table::borrow_mut<address, Table<u64, StakeInfo>>(&mut staking_platform.stake_address_record_table, ctx.sender());
        let mut stake_number_info_table = table::borrow_mut<u64, StakeInfo>(stake_address_record_table, address_stake_counter);
        ...
        // 更新本次提取时间
        stake_number_info_table.last_withdraw_time = current_time.timestamp_ms();
        ...
        let mut vector_day_reward: vector<Coin<COIN>> = table::remove(&mut rewardpool_number_coin_table, address_stake_counter);
        ...
        // 未使用的reward打回奖池
        table::add(&mut rewardpool_number_coin_table, address_stake_counter, vector_day_reward);
